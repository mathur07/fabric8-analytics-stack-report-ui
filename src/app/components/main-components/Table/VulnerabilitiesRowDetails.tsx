...
function VulnerabilitiesRowDetails(props: RowDetailType) {
  const { critical, high, medium, low } = props;
  return (
    <Flex
      key="1"
    ...
    >
      <FlexItem spacer={{ default: "spacerXs" }}>
        <Split hasGutter>
          {/* @ts-ignore */}
          <SplitItem>{critical + high + medium + low}</SplitItem>{" "}
          <Divider
            isVertical
            inset={{
              default: "insetMd",
          ...
          {critical ? (
            <SplitItem>
              <SecurityIcon className="icon-class" color="#7D1007" /> {critical}
            </SplitItem>
          ) : null}
          {high ? (
            <SplitItem>
          ...
          {medium ? (
            <SplitItem>
              <SecurityIcon className="icon-class" color="#EC7A08" /> {medium}
            </SplitItem>
          ) : null}
          {low ? (
            <SplitItem>
