...
export class ReportSummaryUtils {
    ...
    public titleAndDescription: any = {
        ...
        [this.cardTypes.INSIGHTS]: {
            ...
            description:
                "Complementary dependencies that can be added to your application stack or replace current dependencies with alternate one. Click to see details.",
        },
    ...
    public getSecurityReportCard(
        userStackInfo: UserStackInfoModel
    ): MReportSummaryCard {
        ...
        securityCard.reportSummaryDescription = this.titleAndDescription[
            this.cardTypes.SECURITY
        ].description;
        ...
        if (
            ...
            analyzedDependencies = userStackInfo.analyzed_dependencies.filter(
                (info) => {
            ...
            analyzedDependencies.forEach((analyzed) => {
                if (analyzed.security && analyzed.security.length > 0) {
                    let currSecurity: Array<SecurityInformationModel> =
                        analyzed.security;
            ...
            if (maxIssue) {
                let securityColor: string =
                    Number(maxIssue.CVSS) >= 7
                        ? this.colors.security.warning
                        : this.colors.security.moderate;
                ...
                maxIssueEntry.config = {
                    ...
                    footerText:
                        "No. of dependencies with this CVSS Score: " +
                        totalComponentsWithMaxScore,
    ...
    public getInsightsReportCard(
        ...
        insightsCard.reportSummaryDescription = this.titleAndDescription[
            this.cardTypes.INSIGHTS
        ].description;
        insightsCard.reportSummaryContent.infoEntries = [];

        let usageOutliersCount: number = 0,
            companionCount: number = 0;
        if (recommendation) {
            let usage = recommendation.usage_outliers;
            usageOutliersCount = usage ? usage.length : 0;
            companionCount = recommendation.companion
                ? recommendation.companion.length
                : 0;
            ...
            insightsCard.reportSummaryContent.infoEntries.push(
                outliersInsights
            );
            ...
            if (usageOutliersCount > 0) {
                insightsCard.reportSummaryTitle.notificationIcon =
                    "pficon-info";
    ...
    public getLicensesReportCard(
        ...
        licensesCard.reportSummaryDescription = this.titleAndDescription[
            this.cardTypes.LICENSES
        ].description;
        licensesCard.reportSummaryContent.infoEntries = [];

        if (userStackInfo && userStackInfo.license_analysis) {
            ...
            if (stackLicenses) {
                if (stackLicenses.length > 0) {
                    ...
                    if (
                        ...
                        licenseAnalysis.status.toLowerCase() === "failure"
                    ) {
            ...
            let conflictLicenses = this.getConflictLicenseCount(
                licenseAnalysis.conflict_packages
            );
            conflictLicense.infoValue = conflictLicenses
                ? conflictLicenses.length
                : 0;
            ...
            unknownLicense.infoValue = unknownLicenses
            ...
            if (
                ...
                restrictiveLicenses.infoValue = restrictive
                    ? restrictive.length
                    : 0;
                licensesCard.reportSummaryContent.infoEntries.push(
                    restrictiveLicenses
                );
            }
